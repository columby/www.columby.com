...
var SendGridLogger = winston.transports.SendGridLogger = function (options) {
  ...
  this.subject = options.subject || 'Message from Columby files';
...
winston.add(winston.transports.SendGridLogger, {
  ...
  subject: 'Columby files error message',
  level: 'error'
});
  ...
  app.use(expressWinston.errorLogger({
    ...
    transports: [
      new winston.transports.SendGridLogger({
        to: 'admin@columby.com',
        subject: 'Columby files error report: Uncaught exception handler'
      })
    ]
  }));
};
