...
CsvWorker.prototype.start = function(job, callback){
  ...
  function dropTable(cb) {
    logger.debug('Dropping table ' + self._tablename);
  ...
  function getDataFile(cb){
    logger.debug('Downloading data-file to local disk.');
  ...
  function createTable(cb) {
    if (!self._columns){
      return cb('There was an error parsing the columns. ');
    }
